...
const Register = () => {
    ...
    const sendRegister = () => {
        ...
        if (state.userType === "seller" && state.direction.length === 0) {
            alert("Inserte una dirección válida");
            return;
        }

        if (state.userType === "seller") {
            LocationManagement.GetCoordinatesFromAddress(state.direction).then(
                (coordinates) => {
    ...
    const sendRequest = () => {
        axios
            .post(
                `${process.env.REACT_APP_BACKEND_DEFAULT_ROUTE}users/register`,
                {
                    ...state,
                }
            )
