...
import CurrentPositionIconSvg from "./my-position.svg";
import MarkerIconSvg from "./location-seller.svg";
...
function Map() {
  ...
  const [showLocationSnackbar, setShowLocationSnackbar] = React.useState(true);
  ...
  React.useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_DEFAULT_ROUTE}users/mapLocations`)
      .then((res) => {
        setStores(
          res.data.filter(
            (store: StoreType) =>
  ...
  function MapManager() {
    ...
    map.addEventListener("move", () => {
      if (position) {
        setLocationCentered(map.distance(map.getCenter(), position) < 100);
      }
    });
    ...
    return position === null ? null : (
      <Marker position={position} icon={CurrentLocationIcon()}>
        <Circle center={position} radius={radius} color="#4C987B" />
      </Marker>
    );
  }
    ...
    <>
      ...
      >
        <Alert
          ...
          elevation={6}
        >
          No se ha podido obtener tu ubicación. Por favor, comprueba que la
          ubicación está activada en tu dispositivo.
        </Alert>
      </Snackbar>
      <Snackbar
        sx={{
      ...
      {!locationError && !showLocationFoundSnackbar && !showLocationSnackbar && (
        <Zoom
          in={true}
          timeout={transitionDuration}
          style={{
